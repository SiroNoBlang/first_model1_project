...
// 데이터베이스테 저장되어 있는 주소값, 전화번호는 잘라서 다시 나눠서 분배하여 밑에 출력 해야되고
// 직업, 성별은 value 값을 비교해서 일치하는 값을 선택해서 보여줄 수 있도록 정해야된다. 전에 한 자료 찾아보거나 검색해볼것
%>
...
<script type="text/javascript">
	...
	// 확인해본 결과 Retype에만 필수를 걸어두면 불일치가 일어나기 때문에 반드시 입력해야할 필요가 생겨 불일치 일경우 넘어가지 않는데?
	// 그러면 비밀번호 안 바꾸려면 같은 값을 입력해야 되는건가 싶지만 그냥 바꾸는걸로 할까 생각중이다.
	// 굳이 비밀번호를 안바꿀꺼라면 같은 비밀번호를 쳐도 되게 만들어두면 되니깐 그냥 유니크만 하지 않으면 대충 적당히 기능할꺼 같다.
	// 다시 생각해봤는데 빈칸은 안될꺼 같다. 라디오랑 셀렉트는 별 상관 없어서 그건 제외하고 나머지는 다 공백만큼은 피하게 만들자.
	function checkForm() {
		  ...
		  <article>
		  	...
		  	<form action="joinPro.jsp" method="post" id="join" name="fr" onsubmit="return checkForm()">
		  		<fieldset>
		  			...
		  			<input type="text" name="nickname" id="nickname" value="<%=nickname %>" readonly="readonly"><br>
		  			...
		  			<input type="password" name="pass" id="pass" value="<%= %>" onkeyup="checkPass(this.value)">
		  			<span id="checkPassResult"></span>
		  			<br> 			
		  			
		  			<label>Retype Password</label>
		  			<input type="password" name="pass2" value="<%= %>" onkeyup="checkConfirmPass(this.value)" required="required">
		  			...
		  			<input type="text" name="phone1" class="phone" id="phone" value="<%= %>" onkeyup="checkPhone1(this.value)"> -
					<input type="text" name="phone2" class="phone" id="phone" value="<%= %>" onkeyup="checkPhone2(this.value)"> -
					<input type="text" name="phone3" class="phone" id="phone" value="<%= %>" onkeyup="checkPhone3(this.value)">
					<br>
					
					<label>Post code</label>
		  			<input type="text" name="postcode" id="postcode" value="<%= %>" readonly="readonly">
		  			<input type="button" class="dup" onclick="kakaoPostcode()" value="우편번호 찾기"><br>
		  			<label>Address</label>
		  			<input type="text" name="address" id="address" value="<%= %>" size="55" readonly="readonly"><br>
		  			<label>&nbsp;</label>
		  			<input type="text" name="detailAddress" id="detailAddress" value="<%= %>" size="29">
		  			<input type="text" name="extraAddress" id="extraAddress" value="<%= %>" readonly="readonly">
		  			<br>
		  		</fieldset>
