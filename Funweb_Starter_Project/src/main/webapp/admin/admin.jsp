		...
		<article>
			...
			<table id="notice">
				...
				<%for(Master_news_DTO news : newsList) { %>
					<!-- notice_content.jsp 페이지로 이동 -->
					<!-- URL 파라미터로 글번호와 페이지번호 전달 -->
					<tr onclick="location.href='news_content.jsp?num=<%=news.getNum() %>&page=<%=pageNum %>'">
						<td><%=news.getNum() %></td>
			...
			<div id="table_search">
				<form action="newsList_search.jsp" method="get">
			...
			<div id="page_control">
				<%if(pageNum > 1) { // 이전페이지가 존재할 경우 %>
					<a href="admin.jsp?page=<%=pageNum - 1%>">Prev</a>
				...
				<%if(pageNum < maxPage) { // 다음페이지가 존재할 경우 %>
					<a href="admin.jsp?page=<%=pageNum + 1%>">Next</a>
				<%} else { // 이전페이지가 존재하지 않을 경우 %>
					&nbsp;Next
				<%} %>
			</div>
